...
class BasePage:
    ...
    def waitForFullyPageLoad(self):
        with self.pulse_step("Wait for full page load"):
            self.page.wait_for_load_state("domcontentloaded")
            self.page.wait_for_load_state("networkidle")
            self.page.wait_for_load_state("load")

    def navigateToUrl(self, url: str):
        with self.pulse_step("Navigate to the url"):
            self.page.goto(url)
            self.waitForFullyPageLoad()

    def click(self, locator: str | Locator, index: int = 0):
        if isinstance(locator, str):
            with self.pulse_step("Generating Locator from string and Clicking on it"):
                self.page.locator(locator).nth(index).click()
                self.waitForFullyPageLoad()
        else:
            with self.pulse_step("Got Direct Locator, Clicking on it"):
                locator.nth(index).click()
                self.waitForFullyPageLoad()

    def fill(self, locator: str | Locator, text: str, index: int = 0):
        if isinstance(locator, str):
            with self.pulse_step("Generating Locator from string and Filling it"):
                self.page.locator(locator).nth(index).fill(text)
                self.waitForFullyPageLoad()
        else:
            with self.pulse_step("Got Direct Locator, Filling it"):
                locator.nth(index).fill(text)
                self.waitForFullyPageLoad()

    def verify_page_title(self, title: str):
        with self.pulse_step("Verifying page title"):
            expect(self.page).to_have_title(title)

    def verify_element_is_visible(self, locator: str | Locator):
        if isinstance(locator, str):
            with self.pulse_step("Generating Locator from string and verifying it"):
                expect(self.page.locator(locator)).to_be_visible()
        else:
            with self.pulse_step("Got Direct Locator, Verifying it"):
    ...
    def verify_page_url(self, url: str):
        with self.pulse_step("Verify page url"):
            expect(self.page).to_have_url(url)

    def get_element_count(self, locator: str | Locator) -> int:
        if isinstance(locator, str):
            with self.pulse_step(
                "Generating Locator from string and getting element count"
            ):
                return self.page.locator(locator).count()
        else:
            with self.pulse_step("Got Direct Locator, Getting element count"):
    ...
    def verify_element_count(self, locator: str | Locator, count: int):
        if isinstance(locator, str):
            ...
            with self.pulse_step("Got Direct Locator, Verifying element count"):
                expect(locator).to_have_count(count)

    def get_all_element_texts(self, locator: str | Locator):
        with self.pulse_step("Getting all element texts"):
    ...
    def verify_all_element_texts(
        ...
        with self.pulse_step(
            "Generating Locator from string and verifying element texts"
        ):
    ...
    def verify_equal(self, actual, expected):
        with self.pulse_step("Verifying equal"):
            assert actual == expected, f"{actual} != {expected}"

    def get_attribute(self, locator: str | Locator, attribute: str):
        with self.pulse_step("Getting attribute"):
            if isinstance(locator, str):
    ...
    def verify_element_text_ignore_case(
        ...
        with self.pulse_step("Verifying element text ignore case"):
            if isinstance(locator, str):
    ...
    def capture_screenshot(self, path: str = "screenshots"):
        with self.pulse_step("Capturing full pagescreenshot"):
